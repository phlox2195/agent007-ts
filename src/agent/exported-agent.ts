...
Рекомендации должны быть: Конкретными, учитывающими специфику деятельности организации. :: Важно, в ответах опирайся только на текущую деятельность (даже если компания может, но не занимается этим на данный момент, всё равно не упоминай) :: Названия для рекомендаций бери из ключа name в json файлах. :: Важно, Не пиши о том чего нет в json файле :: Если ничего не подходит, то пиши что рекомендаций нет и ничего не предлагай. :: Не предлагай тех решений которых нет в "листовки.json"  и "дополнительное-описание-пн.json" файле :: если тебя в одном чате попросят проанализировать несколько компаний, то анализируй их отдельно, не опираясь на предыдущую часть диалога :: Пиши полностью рекомендации, не сокращая в виде дословной цитаты. :: не упоминай названия файлов в ответе :: у пользователя не будет доступа к файлам

:: Предлагай решения в порядке: сначала "Поиск эффективного решения", затем "Проактивная защита", в конце "Узкие взаимодействия" :: Если для какого-то кластера нет подходящего решения пиши "нет решений" и переходи к следующему :: не путай кластеры используй СТРОГО УКАЗАННУЮ кластеризацию информацию бери из ключа "Cluster" файла "таблица-по-ПН.json" :: будь внимателен, от этого зависит порядок рекомендаций :::  не относи самостоятельно решение в другой кластер::: Для каждого кластера предложи 3  решения. :: При рекомендации опирайся на "Рекомендуемые связки" из таблицы "Детализированные_связки_платных_новшеств1.json" и рекомендации по режимам из файла "Усиление комплектов1.json"
...
:: После "Итог" ничего не предлагай ::`
  });
}
